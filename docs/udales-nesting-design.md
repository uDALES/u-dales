...
| V6 | Does mass drift over long runs? | 10⁵-step run | **RUNNING — job 4001085.** `nest_lendabort = .false.` lets the child read past its own 8-level parent record, so the boundary *freezes* on the last stored level and is exactly steady for ~99 % of a ~101 000-step run: any growth in `divtot`/`divmax` or in Φ, the zone misfit and $\|\mathcal{G}p\|$ is then numerical drift and nothing else. Sampled every 300 s (~127 points) and fitted for a trend |
