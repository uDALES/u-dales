...
> **Current state, so this section can be read without reconstructing its history: the writer's
> default is `prolongation="linear"`, settled by V0b; the guarantee derived above belongs to
> `prolongation="constant"`, which remains available and is the only scheme that has it.** What
> follows records how that was arrived at, including two reversals, because the reasoning matters
...
>
> **What the linear scheme buys is measured; what it costs is not yet.** The mean-flow error falls by
> $1.3\times$ at $r=2$ and $2.3\times$ at $r=4$, the staircase by $6$–$9\times$, the interior TKE deficit
> roughly halves, and post-projection `divmax` ($1.7\times10^{-15}$) and $\Phi$ ($1.8\times10^{-15}$) are
> unchanged. That is a real empirical case for the cases tested.
>
> **A claim made here on 2026-09-08 was withdrawn, and V0c has since reinstated it on proper
> evidence — see §10.5.** With 361 samples per arm and a no-prolongation control, the interior
> pressure norm is identical across pre-projection divergences spanning six orders, and the zone
> norm *falls* as the ratio rises. The original wording and why it was not supportable at the time:
> it said the projection absorbs the extra divergence "locally, at $1$–$3\,\%$ on the pressure
> response". V0b inherited the default diagnostic
> interval (`nest_statint = -1` → `tstatsdump`), so each run emitted only its compulsory **first and
> last** pressure reports — $n=2$ for every point, the first at $t=0.4$ s, outside an averaging window
> that starts at 600 s. Two endpoints are not a time mean, and the endpoint numbers do not support
> locality either: at $r=4$ the *interior* norm rises more ($+11.8\,\%$) than the zone norm
> ($+8.2\,\%$), while the ratio of the two barely moves. Post-projection divergence at round-off is
> what a working projection always gives and settles nothing here. Deciding whether the extra
> divergence stays in the zone needs a run with a real diagnostic interval and the startup discarded.
>
...
>
> *(Historical, superseded by V0b below.)* At this point neither scheme had been validated end to
> end as shipped — V0's production numbers came from `constant` at the old 3 s linear-in-time
> cadence, and `linear` had never run at production size — so the default was reverted to `constant`
> pending measurement.
...
**A second window correction (2026-09-08).** The driving-parent profile was accumulated over
*every* stored level while the reference and child statistics use only `stats_start <= t <=
runtime`, so the "of which parent's" attribution carried a window mismatch on top of the filtering
difference — for V0b, 4800 parent levels against 600 reference samples. The manifest now carries a
separate validation profile over the analysis window (and `analyse_v0` refuses the comparison if
the windows disagree), while the seeding profile still uses every level. Recomputed on V0b, the
attribution moves by about a point aloft and three in the canopy — e.g. at $r=2$ the parent's own
deficit above $z/h=2$ goes $-11.70 \to -10.28\,\%$ and its canopy figure $-23.12 \to -26.84\,\%$ —
so no conclusion changes, but the numbers are now like-for-like.
...
($9.470\times10^{-8}$ at $r=2$), which is the strongest confirmation §1.3 has. Second, **the mean-flow comparison needs a
matched-resolution control at the same window, and with one the picture is undramatic.** Two
corrections apply to what was written here on 2026-09-08. Both are the reviewer's.

*The two numbers being compared were different statistics.* V1 converged's celebrated
$0.00826\,u_\star$ is a **profile RMS**; criterion A is the **maximum interior slab RMS**, a
different and larger quantity (`analyse.py:521` and `:692`). V1's own criterion A is $0.0386$, which
clears the $0.05$ bound by $1.29\times$, not by the factor of six implied by setting $0.05$ against
the profile RMS. The refined children's *profile* RMS is $0.0159$ ($r=2$) and $0.0136$ ($r=4$) —
comfortably inside $0.05$.

*And the control was missing.* C0c's `cr0.5` child is matched-resolution, driven by the same fine
parent 960, at the same cadence and interpolant, over the same 1800 s window with the same 600
samples. Its criterion A is $0.0833$ — **it fails the bound too**:

| case | window | profile RMS | criterion A |
|---|---|---|---|
| V1 converged, matched | 10 191 s | $0.0083$ | $0.0386$ (passes) |
| C0c `cr0.5`, matched | 1800 s | $0.0077$ | $0.0833$ |
| V0b $r=2$, linear | 1800 s | $0.0159$ | $0.0978$ |
| V0b $r=4$, linear | 1800 s | $0.0136$ | $0.1209$ |

So refinement costs about $17\,\%$ and $45\,\%$ on criterion A relative to a matched control at the
same window, not a collapse from success to failure; the absolute failure is shared with the
control and is a property of the short window. In dimensional terms these are $\approx0.039$ and
$0.048$ m/s, not "$9.8$ and $12.1\,\%$ errors in the wind".

One further caution, also the reviewer's, against an argument made here earlier: the parent's
half-window spread is **not** a valid uncertainty for the paired child-minus-parent difference, so
"the error sits below the sampling floor" was the wrong way to retract the wrong claim. (And of the
four values, $0.280$ is only $2.2\times$ below that floor, not five to six.) Uncertainty here should
come from temporal blocks of the **paired differences**, keeping the spatial structure. Whether
criterion A is genuinely violated at $r>1$ needs longer windows or repeat realisations — it is not
established either way, and nothing in this run justifies replacing the reconstruction to fix it.

What the profiles do show, and what settles the natural objection that a mean flow ought to be
easier to carry than turbulence: **above the canopy the mean is reproduced extremely well.** At
$r=2$ with the shipped scheme, $|\Delta\langle u\rangle| \le 0.02\,u_\star$ for every level above
$z/h=2$, and mostly nearer $0.005$. The residual sits low down — peaking at $0.056\,u_\star$ at
$z/h=0.31$ for $r=2$ and $0.059\,u_\star$ at roof height for $r=4$ — which is exactly where a 4 m or
8 m parent cannot represent 16 m cubes and the box filter mixes solid with fluid. So the mean *is*
easier than the turbulence, as expected; criterion A is a maximum over the whole column, so it
reports the canopy, while the TKE figures quoted alongside it are measured above $z/h=2$. The two
were never describing the same region. **To test criterion A at $r>1$ properly, V0 needs a
converged-length child window** — V1's lesson, repeated.

**V0c — refinement at a converged window, with a matched control (job 4004496, 8 h 35).** The
experiment the 8 September review asked for: r = 1, 2 and 4 through one pipeline, the shipped
`linear` prolongation, 0.5 s cadence with the cubic, and **the same 10 200 s window V1 converged
used** — 3400 samples against V0b's 600. `nest_statint = 30 s` gives 361 pressure samples per run
instead of V0b's two. At r = 1 the coarsening degenerates to the identity, so that arm is a plain
slab cut and the writer never touches the prolongation: a true control.

| | $r=1$ (control) | $r=2$ | $r=4$ |
|---|---|---|---|
| profile RMS $[u_\star]$ | $0.0027$ | $0.0104$ | $0.0195$ |
| **criterion A** $[u_\star]$ | $\mathbf{0.0392}$ | $\mathbf{0.0479}$ | $0.0746$ |
| the same at 1800 s (V0b / C0c) | $0.0833$ | $0.0978$ | $0.1209$ |
| resolved TKE above $z/h=2$ | $-1.32\,\%$ | $-2.18\,\%$ | $-9.21\,\%$ |
| — its parent's own deficit | $0.00\,\%$ | $-9.20\,\%$ | $-22.82\,\%$ |
...
*The 1800 s failures were a sampling artefact, and refinement to $r=2$ meets criterion A.* Every
value roughly halves on the longer window ($2.1\times$, $2.0\times$, $1.6\times$), as $1/\sqrt N$
predicts. The matched control lands at $0.0392$ — against V1 converged's independently measured
$0.0386$, which is as good a cross-check as the campaign has produced — and **$r=2$ passes at
$0.0479$**. $r=4$ does not, at $0.0746$. So the bound is met at a refinement ratio of two and
missed by about $50\,\%$ at four, and the earlier "refinement fails the mean-flow standard" reading
was an artefact of a short window throughout.

*A refined child improves on its parent, substantially.* At $r=2$ the child carries $-2.18\,\%$
where its driving parent carries $-9.20\,\%$; at $r=4$, $-9.21\,\%$ against $-22.82\,\%$. It
recovers roughly **7 and 14 points** of turbulence its parent never had. This is the positive
refinement result the campaign had been missing, and it retires the earlier claim that a child
"regenerates none of what an $r=4$ parent never resolved" — that was measured at the old cadence
with the staircase-prone reconstruction, and both mattered.

*The linear prolongation's divergence does not reach the interior.* This is the question §1.3
withdrew for want of evidence. The interior pressure norm is $0.01162$, $0.01161$, $0.01156$ across
the three arms — a spread of $0.5\,\%$ against a standard deviation of $24\,\%$ of the mean, i.e.
indistinguishable — while the pre-projection child divergence spans $2.7\times10^{-7}$ (no
prolongation at all) to $0.363$, six orders. The zone norm does differ, but it *falls* as $r$ rises
($0.0323\to0.0262$, about $3\sigma$), the opposite of what a divergence-driven cost would do; it
tracks the smoothness of the imposed target, not its divergence. **So the extra divergence is
