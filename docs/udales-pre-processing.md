...
Each row describes a block, with the first 6 columns giving the lower (l) and upper (u) limit in each direction (i,j,k), in the order: il, iu, jl, ju, kl, ku.
The next 5 columns give the facets that make up the faces of the block, in the order: top, west, east, north, south. (Note east is assumed to be aligned with the i-direction).
