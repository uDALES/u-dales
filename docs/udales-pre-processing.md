...
Python preprocessing and the legacy MATLAB route treat the urban surface mesh
as an open radiative domain:
surface-to-surface view factors plus each facet's sky view factor must sum to one.
If numerical or geometric edge cases make a View3D row materially exceed one,
outside the validation tolerance, the default
`repair` policy reduces reciprocal interchange areas conservatively and assigns
the remaining fraction to sky. The correction preserves non-negativity,
reciprocity, and sparse storage; it never proceeds with a materially overfull
matrix unchanged.
A warning records the correction; Python additionally writes a
`view3d_repair.###.json` audit report. Automatic repair stops instead of masking
a gross geometry problem when the maximum row sum exceeds 2, materially
overfull facets exceed 5% of total facet area, the correction removes more than
1% of total surface exchange area, or the reciprocity L1 error exceeds 1%. Set
`UDALES_VIEW_FACTOR_POLICY=strict` before `write_inputs.sh -p` to reject any
material row-closure violation rather than repair it.
