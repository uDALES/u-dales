...
A third restart file, `initf<ntrun>.<expnr>`, holds the facet state. It is written whenever `nfcts > 0` and at least one of `lEB`, `iwalltemp = 2`, `iwallmom = 2` or `iwallmoist = 2` is active. Unlike the flow fields, the facet arrays are global rather than domain-decomposed, so this file is written by rank 0 only and carries no `_<px>_<py>` rank fields in its name. It is self-describing: record 1 is an integer header (format version, `nfcts`, `nfaclyrs`), followed by the wall temperature profile `facT` (`nfcts` facets by `nfaclyrs + 1` layer interfaces), the soil moisture `facwsoil` (only when `lEB`), the saturation humidity `facqsat`, the relative humidity `fachurel`, the green-roof resistance factors `facf`, and finally the current time. Without this file a warm start silently re-initialises the facet temperatures from `Tfacinit.inp.<expnr>`, so the surface energy balance does not continue from the checkpoint.

To resume from a restart file, set `lwarmstart = .true.` (or `lstratstart = .true.`) and point `startfile` at the `initd...` filename of the desired restart point (the trailing digits of `startfile` must match the experiment's `iexpnr`); the matching `inits...` and `initf...` files, if present, are read automatically. If `initf...` is missing, uDALES prints a warning and falls back to `Tfacinit.inp.<expnr>` rather than aborting, so restart files written by older versions still work for the flow fields. If it is present but its `nfcts` or `nfaclyrs` do not match the current `namoptions`, the run aborts. Set `lreadfacT = .false.` to deliberately re-seed the facet temperatures from `Tfacinit.inp.<expnr>` on a warm start. See the `RUN` and `SCALARS` namelists on the [Configuration](udales-namoptions-overview.md) page for the exact parameter definitions.
