...
def parent_sections(preset: Preset, *, warmstart: bool, startfile: str,
                    nestdump_child: Optional[Preset] = None) -> "OrderedDict":
    """Namelist content for one phase of the parent run.

    What the production phase writes is ``preset.parent_output``: full field
    dumps (``&OUTPUT``), the child's band only (``&NESTDUMP``,
    ``src/modnestdump.f90``), or both.  ``nestdump_child`` sizes the band for a
    child other than the preset's own -- a V0 driver dumping for the fine
    child it will drive -- see :meth:`config.Preset.nestdump_sections`.
    """
    nr = preset.parent_expnr
    production = warmstart
    nestdump = OrderedDict([("lnestdump", False)])
    if production and preset.writes_nestdump:
        nestdump = preset.nestdump_sections(nestdump_child)
    return OrderedDict([
        ("RUN", OrderedDict([
            ("iexpnr", int(nr)),
            ("runmode", 1),
...
def build(outdir: Path, preset: Preset, ibm_backend: str = "auto",
          ...
          nestdump_child: Optional[Preset] = None) -> Path:
    """Create and preprocess the parent case; return its directory.

    ``nestdump_child`` is passed on to :func:`parent_sections`: the child the
    ``&NESTDUMP`` band is sized for when it is not the preset's own.
