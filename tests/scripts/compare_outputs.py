...
# FIXME: For now, we only compare wind vectors
quantities = ['u', 'v', 'w']
...
def calc_diff(path_to_ds_a: Path, path_to_ds_b: Path, quantities: List[str]) -> List[nc.Dataset]:
    ...
    for quantity in quantities:
        ...
        print(f'Max approximate error for {quantity}: {diff.max()} m/s')
        print(
            f'Standard deviation of approximate error for {quantity}: {diff.std()} m/s')
        diffs.append(diff)
    return diffs


def plot(diffs: List[nc.Dataset], path_to_fig_dir: Path) -> None:
    plt.boxplot(diffs)
    plt.xticks([1, 2, 3], quantities)
    plt.ylabel('Approximation error in m/s')
    plt.xlabel('Quantity')
