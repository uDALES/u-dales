...
contains
    ...
    if (lpurif .eqv. .false.) return
      ...
      use modglobal,  only : ib,ie,jb,je,kb,ke,ih,xf,xh,zh,purif,npurif,lpurif,jtot,Qpu,epu,dy,nsv
      ...
      do n = 1, npurif
        ...
        kl = purif(n,5)
        ku = purif(n,6)
        jl = purif(n,3) - myid*jtot/nprocs
        ju = purif(n,4) - myid*jtot/nprocs
        ...
        ku = purif(n,6)
        jl = purif(n,3) - myid*jtot/nprocs
        ju = purif(n,4) + 1 - myid*jtot/nprocs
        if (ju < jb .or. jl > je) then
          cycle
        else
          if (ju > je) ju=je
