...
implicit none
private
public :: initpois,poisson,exitpois,p,rhs
save

  real,allocatable,target :: p(:,:,:)   ! difference between p at previous and new step (p = P_new - P_old)
  real,allocatable,target :: rhs(:,:,:)   ! rhs of poisson equation
...
contains
  subroutine initpois
    ...
    allocate(rhs(ib:ie,jb:je,kb:ke))

  end subroutine initpois

  subroutine poisson
    use modglobal, only : ib,ie,jb,je,ih,kb,ke,kh,kmax,dxh,dxf,dy,dzf,dzh,linoutflow,iinletgen,ipoiss,POISS_FFT,POISS_CYC
