...
module modstartup
   ...
   use modglobal, mg_rv => rv, json_input
   use modsurfdata
   use modfields
   use modpois
   use modboundary
   ...
   subroutine readjsonconfig
      ...
      if (myid == 0) then
         ...
         call read_run_json(json, found_any)
         call read_domain_json(json, found_any)
         call read_physics_json(json, found_any)
         call read_dynamics_json(json, found_any)
         call read_bc_json(json, found_any)
         call read_driver_json(json, found_any)
         call read_energybalance_json(json, found_any)
         call read_heatpump_json(json, found_any)
         call read_inlet_json(json, found_any)
         call read_output_json(json, found_any)
         call read_purifs_json(json, found_any)
         call read_scalars_json(json, found_any)
         call read_trees_json(json, found_any)
         call read_walls_json(json, found_any)
   ...
   subroutine read_run_json(json, found_any)
      !-----------------------------------------------------------------|
      ...
      if (found) then; iexpnr = temp_int; found_any = .true.; end if
      
      call json%get('RUN.lwarmstart', temp_logical, found)
      if (found) then; lwarmstart = temp_logical; found_any = .true.; end if
      
      call json%get('RUN.lstratstart', temp_logical, found)
      if (found) then; lstratstart = temp_logical; found_any = .true.; end if
      
      call json%get('RUN.startfile', temp_string, found)
      if (found) then; startfile = temp_string; found_any = .true.; end if
      
      call json%get('RUN.runtime', temp_real, found)
      if (found) then; runtime = temp_real; found_any = .true.; end if
      
      call json%get('RUN.trestart', temp_real, found)
      if (found) then; trestart = temp_real; found_any = .true.; end if
      
      call json%get('RUN.dtmax', temp_real, found)
      if (found) then; dtmax = temp_real; found_any = .true.; end if
      
      call json%get('RUN.ladaptive', temp_logical, found)
      if (found) then; ladaptive = temp_logical; found_any = .true.; end if
      
      call json%get('RUN.courant', temp_real, found)
      if (found) then; courant = temp_real; found_any = .true.; end if
      
      call json%get('RUN.diffnr', temp_real, found)
      if (found) then; diffnr = temp_real; found_any = .true.; end if
      
      call json%get('RUN.author', temp_string, found)
      if (found) then; author = temp_string; found_any = .true.; end if
      
      call json%get('RUN.lrandomize', temp_logical, found)
      if (found) then; lrandomize = temp_logical; found_any = .true.; end if
      
      call json%get('RUN.irandom', temp_int, found)
      if (found) then; irandom = temp_int; found_any = .true.; end if
      
      call json%get('RUN.randu', temp_real, found)
      if (found) then; randu = temp_real; found_any = .true.; end if
      
      call json%get('RUN.randthl', temp_real, found)
      if (found) then; randthl = temp_real; found_any = .true.; end if
      
      call json%get('RUN.randqt', temp_real, found)
      if (found) then; randqt = temp_real; found_any = .true.; end if
      
      call json%get('RUN.krand', temp_int, found)
      if (found) then; krand = temp_int; found_any = .true.; end if
      
      call json%get('RUN.libm', temp_logical, found)
      if (found) then; libm = temp_logical; found_any = .true.; end if
      
      call json%get('RUN.lles', temp_logical, found)
      if (found) then; lles = temp_logical; found_any = .true.; end if
      
      call json%get('RUN.lper2inout', temp_logical, found)
      if (found) then; lper2inout = temp_logical; found_any = .true.; end if
      
      call json%get('RUN.lreadmean', temp_logical, found)
      if (found) then; lreadmean = temp_logical; found_any = .true.; end if
      
      call json%get('RUN.lwalldist', temp_logical, found)
      if (found) then; lwalldist = temp_logical; found_any = .true.; end if
      ...
      if (found) then; runmode = temp_int; found_any = .true.; end if
   ...
   subroutine read_domain_json(json, found_any)
      !-----------------------------------------------------------------|
      ...
      if (found) then; itot = temp_int; found_any = .true.; end if
      
      call json%get('DOMAIN.jtot', temp_int, found)
      if (found) then; jtot = temp_int; found_any = .true.; end if
      
      call json%get('DOMAIN.ktot', temp_int, found)
      if (found) then; ktot = temp_int; found_any = .true.; end if
      
      call json%get('DOMAIN.xlen', temp_real, found)
      if (found) then; xlen = temp_real; found_any = .true.; end if
      
      call json%get('DOMAIN.ylen', temp_real, found)
      if (found) then; ylen = temp_real; found_any = .true.; end if
      
      call json%get('DOMAIN.xlat', temp_real, found)
      if (found) then; xlat = temp_real; found_any = .true.; end if
      
      call json%get('DOMAIN.xlon', temp_real, found)
      if (found) then; xlon = temp_real; found_any = .true.; end if
      
      call json%get('DOMAIN.xday', temp_real, found)
      if (found) then; xday = temp_real; found_any = .true.; end if
      
      call json%get('DOMAIN.xtime', temp_real, found)
      if (found) then; xtime = temp_real; found_any = .true.; end if
      
      call json%get('DOMAIN.ksp', temp_int, found)
      if (found) then; ksp = temp_int; found_any = .true.; end if
   ...
   subroutine read_physics_json(json, found_any)
      !-----------------------------------------------------------------|
      ...
      if (found) then; ps = temp_real; found_any = .true.; end if
      
      call json%get('PHYSICS.ltempeq', temp_logical, found)
      if (found) then; ltempeq = temp_logical; found_any = .true.; end if
      
      call json%get('PHYSICS.lbuoyancy', temp_logical, found)
      if (found) then; lbuoyancy = temp_logical; found_any = .true.; end if
      
      call json%get('PHYSICS.lmoist', temp_logical, found)
      if (found) then; lmoist = temp_logical; found_any = .true.; end if
      
      call json%get('PHYSICS.lcoriol', temp_logical, found)
      if (found) then; lcoriol = temp_logical; found_any = .true.; end if
      
      call json%get('PHYSICS.igrw_damp', temp_int, found)
      if (found) then; igrw_damp = temp_int; found_any = .true.; end if
      
      call json%get('PHYSICS.lprofforc', temp_logical, found)
      if (found) then; lprofforc = temp_logical; found_any = .true.; end if
      
      call json%get('PHYSICS.ifixuinf', temp_int, found)
      if (found) then; ifixuinf = temp_int; found_any = .true.; end if
      
      call json%get('PHYSICS.lvinf', temp_logical, found)
      if (found) then; lvinf = temp_logical; found_any = .true.; end if
      
      call json%get('PHYSICS.tscale', temp_real, found)
      if (found) then; tscale = temp_real; found_any = .true.; end if
      
      call json%get('PHYSICS.dpdx', temp_real, found)
      if (found) then; dpdx = temp_real; found_any = .true.; end if
      
      call json%get('PHYSICS.lnudge', temp_logical, found)
      if (found) then; lnudge = temp_logical; found_any = .true.; end if
      
      call json%get('PHYSICS.lnudgevel', temp_logical, found)
      if (found) then; lnudgevel = temp_logical; found_any = .true.; end if
      
      call json%get('PHYSICS.ltimedeplw', temp_logical, found)
      if (found) then; ltimedeplw = temp_logical; found_any = .true.; end if
      
      call json%get('PHYSICS.ltimedepnudge', temp_logical, found)
      if (found) then; ltimedepnudge = temp_logical; found_any = .true.; end if
      
      call json%get('PHYSICS.ltimedepsurf', temp_logical, found)
      if (found) then; ltimedepsurf = temp_logical; found_any = .true.; end if
      
      call json%get('PHYSICS.ltimedepsw', temp_logical, found)
      if (found) then; ltimedepsw = temp_logical; found_any = .true.; end if
      
      call json%get('PHYSICS.luoutflowr', temp_logical, found)
      if (found) then; luoutflowr = temp_logical; found_any = .true.; end if
      
      call json%get('PHYSICS.luvolflowr', temp_logical, found)
      if (found) then; luvolflowr = temp_logical; found_any = .true.; end if
      
      call json%get('PHYSICS.lvoutflowr', temp_logical, found)
      if (found) then; lvoutflowr = temp_logical; found_any = .true.; end if
      
      call json%get('PHYSICS.lvvolflowr', temp_logical, found)
      if (found) then; lvvolflowr = temp_logical; found_any = .true.; end if
      
      call json%get('PHYSICS.nnudge', temp_int, found)
      if (found) then; nnudge = temp_int; found_any = .true.; end if
      
      call json%get('PHYSICS.ntimedeplw', temp_int, found)
      if (found) then; ntimedeplw = temp_int; found_any = .true.; end if
      
      call json%get('PHYSICS.ntimedepnudge', temp_int, found)
      if (found) then; ntimedepnudge = temp_int; found_any = .true.; end if
      
      call json%get('PHYSICS.ntimedepsurf', temp_int, found)
      if (found) then; ntimedepsurf = temp_int; found_any = .true.; end if
      
      call json%get('PHYSICS.ntimedepsw', temp_int, found)
      if (found) then; ntimedepsw = temp_int; found_any = .true.; end if
      
      call json%get('PHYSICS.tnudge', temp_real, found)
      if (found) then; tnudge = temp_real; found_any = .true.; end if
      
      call json%get('PHYSICS.uflowrate', temp_real, found)
      if (found) then; uflowrate = temp_real; found_any = .true.; end if
      
      call json%get('PHYSICS.vflowrate', temp_real, found)
      if (found) then; vflowrate = temp_real; found_any = .true.; end if
   ...
   subroutine read_dynamics_json(json, found_any)
      !-----------------------------------------------------------------|
      ...
      if (found) then; iadv_mom = temp_int; found_any = .true.; end if
      
      call json%get('DYNAMICS.iadv_tke', temp_int, found)
      if (found) then; iadv_tke = temp_int; found_any = .true.; end if
      
      call json%get('DYNAMICS.iadv_thl', temp_int, found)
      if (found) then; iadv_thl = temp_int; found_any = .true.; end if
      
      call json%get('DYNAMICS.iadv_qt', temp_int, found)
      if (found) then; iadv_qt = temp_int; found_any = .true.; end if
      
      call json%get('DYNAMICS.iadv_sv', temp_int, found)
      if (found) then; iadv_sv = temp_int; found_any = .true.; end if
      
      call json%get('DYNAMICS.ipoiss', temp_int, found)
      if (found) then; ipoiss = temp_int; found_any = .true.; end if
      
      call json%get('DYNAMICS.lqlnr', temp_logical, found)
      if (found) then; lqlnr = temp_logical; found_any = .true.; end if
   ...
   subroutine read_bc_json(json, found_any)
      !-----------------------------------------------------------------|
      ...
      if (found) then; bcbotm = temp_int; found_any = .true.; end if
      
      call json%get('BC.bcbotq', temp_int, found)
      if (found) then; bcbotq = temp_int; found_any = .true.; end if
      
      call json%get('BC.bcbots', temp_int, found)
      if (found) then; bcbots = temp_int; found_any = .true.; end if
      
      call json%get('BC.bcbott', temp_int, found)
      if (found) then; bcbott = temp_int; found_any = .true.; end if
      
      call json%get('BC.bcqfxm', temp_real, found)
      if (found) then; bcqfxm = temp_real; found_any = .true.; end if
      
      call json%get('BC.bcqfxp', temp_real, found)
      if (found) then; bcqfxp = temp_real; found_any = .true.; end if
      
      call json%get('BC.bcqfym', temp_real, found)
      if (found) then; bcqfym = temp_real; found_any = .true.; end if
      
      call json%get('BC.bcqfyp', temp_real, found)
      if (found) then; bcqfyp = temp_real; found_any = .true.; end if
      
      call json%get('BC.bcqfz', temp_real, found)
      if (found) then; bcqfz = temp_real; found_any = .true.; end if
      
      call json%get('BC.bctfxm', temp_real, found)
      if (found) then; bctfxm = temp_real; found_any = .true.; end if
      
      call json%get('BC.bctfxp', temp_real, found)
      if (found) then; bctfxp = temp_real; found_any = .true.; end if
      
      call json%get('BC.bctfym', temp_real, found)
      if (found) then; bctfym = temp_real; found_any = .true.; end if
      
      call json%get('BC.bctfyp', temp_real, found)
      if (found) then; bctfyp = temp_real; found_any = .true.; end if
      
      call json%get('BC.bctfz', temp_real, found)
      if (found) then; bctfz = temp_real; found_any = .true.; end if
      
      call json%get('BC.bctopm', temp_int, found)
      if (found) then; bctopm = temp_int; found_any = .true.; end if
      
      call json%get('BC.bctopq', temp_int, found)
      if (found) then; bctopq = temp_int; found_any = .true.; end if
      
      call json%get('BC.bctops', temp_int, found)
      if (found) then; bctops = temp_int; found_any = .true.; end if
      
      call json%get('BC.bctopt', temp_int, found)
      if (found) then; bctopt = temp_int; found_any = .true.; end if
      
      call json%get('BC.bcxm', temp_int, found)
      if (found) then; bcxm = temp_int; found_any = .true.; end if
      
      call json%get('BC.bcxq', temp_int, found)
      if (found) then; bcxq = temp_int; found_any = .true.; end if
      
      call json%get('BC.bcxs', temp_int, found)
      if (found) then; bcxs = temp_int; found_any = .true.; end if
      
      call json%get('BC.bcxt', temp_int, found)
      if (found) then; bcxt = temp_int; found_any = .true.; end if
      
      call json%get('BC.bcym', temp_int, found)
      if (found) then; bcym = temp_int; found_any = .true.; end if
      
      call json%get('BC.bcyq', temp_int, found)
      if (found) then; bcyq = temp_int; found_any = .true.; end if
      
      call json%get('BC.bcys', temp_int, found)
      if (found) then; bcys = temp_int; found_any = .true.; end if
      
      call json%get('BC.bcyt', temp_int, found)
      if (found) then; bcyt = temp_int; found_any = .true.; end if
      
      call json%get('BC.bczp', temp_int, found)
      if (found) then; bczp = temp_int; found_any = .true.; end if
      
      call json%get('BC.ds', temp_real, found)
      if (found) then; ds = temp_real; found_any = .true.; end if
      
      call json%get('BC.qt_top', temp_real, found)
      if (found) then; qt_top = temp_real; found_any = .true.; end if
      
      call json%get('BC.qts', temp_real, found)
      if (found) then; qts = temp_real; found_any = .true.; end if
      
      call json%get('BC.thl_top', temp_real, found)
      if (found) then; thl_top = temp_real; found_any = .true.; end if
      
      call json%get('BC.thls', temp_real, found)
      if (found) then; thls = temp_real; found_any = .true.; end if
      
      call json%get('BC.wqsurf', temp_real, found)
      if (found) then; wqsurf = temp_real; found_any = .true.; end if
      
      call json%get('BC.wtsurf', temp_real, found)
      if (found) then; wtsurf = temp_real; found_any = .true.; end if
      
      call json%get('BC.wttop', temp_real, found)
      if (found) then; wttop = temp_real; found_any = .true.; end if
      
      call json%get('BC.z0', temp_real, found)
      if (found) then; z0 = temp_real; found_any = .true.; end if
      
      call json%get('BC.z0h', temp_real, found)
      if (found) then; z0h = temp_real; found_any = .true.; end if
      
      call json%get('BC.wsvsurfdum', temp_real, found)
      if (found) then; wsvsurfdum = temp_real; found_any = .true.; end if
      
      call json%get('BC.wsvtopdum', temp_real, found)
      if (found) then; wsvtopdum = temp_real; found_any = .true.; end if
   ...
   subroutine read_driver_json(json, found_any)
      !-----------------------------------------------------------------|
      ...
      if (found) then; chunkread_size = temp_int; found_any = .true.; end if
      
      call json%get('DRIVER.driverjobnr', temp_int, found)
      if (found) then; driverjobnr = temp_int; found_any = .true.; end if
      
      call json%get('DRIVER.driverstore', temp_logical, found)
      if (found) then; driverstore = temp_logical; found_any = .true.; end if
      
      call json%get('DRIVER.dtdriver', temp_real, found)
      if (found) then; dtdriver = temp_real; found_any = .true.; end if
      
      call json%get('DRIVER.iangledeg', temp_int, found)
      if (found) then; iangledeg = temp_int; found_any = .true.; end if
      
      call json%get('DRIVER.idriver', temp_int, found)
      if (found) then; idriver = temp_int; found_any = .true.; end if
      
      call json%get('DRIVER.iplane', temp_int, found)
      if (found) then; iplane = temp_int; found_any = .true.; end if
      
      call json%get('DRIVER.lchunkread', temp_logical, found)
      if (found) then; lchunkread = temp_logical; found_any = .true.; end if
      
      call json%get('DRIVER.tdriverstart', temp_real, found)
      if (found) then; tdriverstart = temp_real; found_any = .true.; end if
   ...
   subroutine read_energybalance_json(json, found_any)
      !-----------------------------------------------------------------|
      ...
      if (found) then; bldt = temp_real; found_any = .true.; end if
      
      call json%get('ENERGYBALANCE.dteb', temp_real, found)
      if (found) then; dteb = temp_real; found_any = .true.; end if
      
      call json%get('ENERGYBALANCE.flrt', temp_real, found)
      if (found) then; flrt = temp_real; found_any = .true.; end if
      
      call json%get('ENERGYBALANCE.fraction', temp_real, found)
      if (found) then; fraction = temp_real; found_any = .true.; end if
      
      call json%get('ENERGYBALANCE.grlai', temp_real, found)
      if (found) then; grlai = temp_real; found_any = .true.; end if
      
      call json%get('ENERGYBALANCE.lconstw', temp_logical, found)
      if (found) then; lconstw = temp_logical; found_any = .true.; end if
      
      call json%get('ENERGYBALANCE.leb', temp_logical, found)
      if (found) then; leb = temp_logical; found_any = .true.; end if
      
      call json%get('ENERGYBALANCE.lfactlyrs', temp_logical, found)
      if (found) then; lfactlyrs = temp_logical; found_any = .true.; end if
      
      call json%get('ENERGYBALANCE.lperiodicebcorr', temp_logical, found)
      if (found) then; lperiodicebcorr = temp_logical; found_any = .true.; end if
      
      call json%get('ENERGYBALANCE.lvfsparse', temp_logical, found)
      if (found) then; lvfsparse = temp_logical; found_any = .true.; end if
      
      call json%get('ENERGYBALANCE.lwriteebfiles', temp_logical, found)
      if (found) then; lwriteebfiles = temp_logical; found_any = .true.; end if
      
      call json%get('ENERGYBALANCE.nfaclyrs', temp_int, found)
      if (found) then; nfaclyrs = temp_int; found_any = .true.; end if
      
      call json%get('ENERGYBALANCE.nnz', temp_int, found)
      if (found) then; nnz = temp_int; found_any = .true.; end if
      
      call json%get('ENERGYBALANCE.rsmin', temp_real, found)
      if (found) then; rsmin = temp_real; found_any = .true.; end if
      
      call json%get('ENERGYBALANCE.sinkbase', temp_real, found)
      if (found) then; sinkbase = temp_real; found_any = .true.; end if
      
      call json%get('ENERGYBALANCE.skylw', temp_real, found)
      if (found) then; skylw = temp_real; found_any = .true.; end if
      
      call json%get('ENERGYBALANCE.wfc', temp_real, found)
      if (found) then; wfc = temp_real; found_any = .true.; end if
      
      call json%get('ENERGYBALANCE.wgrmax', temp_real, found)
      if (found) then; wgrmax = temp_real; found_any = .true.; end if
      
      call json%get('ENERGYBALANCE.wsoil', temp_real, found)
      if (found) then; wsoil = temp_real; found_any = .true.; end if
      
      call json%get('ENERGYBALANCE.wwilt', temp_real, found)
      if (found) then; wwilt = temp_real; found_any = .true.; end if
   ...
   subroutine broadcast_energybalance_parameters()
      ...
      !-----------------------------------------------------------------|
      ! Note: No variables are currently broadcast for ENERGYBALANCE
   end subroutine broadcast_energybalance_parameters

   subroutine read_heatpump_json(json, found_any)
      !-----------------------------------------------------------------|
      ...
      if (found) then; lheatpump = temp_logical; found_any = .true.; end if
      
      call json%get('HEATPUMP.lfan_hp', temp_logical, found)
      if (found) then; lfan_hp = temp_logical; found_any = .true.; end if
      
      call json%get('HEATPUMP.nhppoints', temp_int, found)
      if (found) then; nhppoints = temp_int; found_any = .true.; end if
      
      call json%get('HEATPUMP.Q_dot_hp', temp_real, found)
      if (found) then; Q_dot_hp = temp_real; found_any = .true.; end if
      
      call json%get('HEATPUMP.QH_dot_hp', temp_real, found)
      if (found) then; QH_dot_hp = temp_real; found_any = .true.; end if
   ...
   subroutine read_inlet_json(json, found_any)
      !-----------------------------------------------------------------|
      ...
      if (found) then; di = temp_real; found_any = .true.; end if
      
      call json%get('INLET.dti', temp_real, found)
      if (found) then; dti = temp_real; found_any = .true.; end if
      
      call json%get('INLET.inletav', temp_int, found)
      if (found) then; inletav = temp_int; found_any = .true.; end if
      
      call json%get('INLET.lfixinlet', temp_logical, found)
      if (found) then; lfixinlet = temp_logical; found_any = .true.; end if
      
      call json%get('INLET.lfixutauin', temp_logical, found)
      if (found) then; lfixutauin = temp_logical; found_any = .true.; end if
      
      call json%get('INLET.linletra', temp_logical, found)
      if (found) then; linletra = temp_logical; found_any = .true.; end if
      
      call json%get('INLET.lreadminl', temp_logical, found)
      if (found) then; lreadminl = temp_logical; found_any = .true.; end if
      
      call json%get('INLET.lstoreplane', temp_logical, found)
      if (found) then; lstoreplane = temp_logical; found_any = .true.; end if
      
      call json%get('INLET.lwallfunc', temp_logical, found)
      if (found) then; lwallfunc = temp_logical; found_any = .true.; end if
      
      call json%get('INLET.uinf', temp_real, found)
      if (found) then; uinf = temp_real; found_any = .true.; end if
      
      call json%get('INLET.vinf', temp_real, found)
      if (found) then; vinf = temp_real; found_any = .true.; end if
   ...
   subroutine read_output_json(json, found_any)
      !-----------------------------------------------------------------|
      ...
      if (found) then; lfielddump = temp_logical; found_any = .true.; end if
      
      call json%get('OUTPUT.tfielddump', temp_real, found)
      if (found) then; tfielddump = temp_real; found_any = .true.; end if
      
      call json%get('OUTPUT.fieldvars', temp_string, found)
      if (found) then; fieldvars = temp_string; found_any = .true.; end if
      
      call json%get('OUTPUT.tsample', temp_real, found)
      if (found) then; tsample = temp_real; found_any = .true.; end if
      
      call json%get('OUTPUT.tstatsdump', temp_real, found)
      if (found) then; tstatsdump = temp_real; found_any = .true.; end if
      
      call json%get('OUTPUT.tstatstart', temp_real, found)
      if (found) then; tstatstart = temp_real; found_any = .true.; end if
      
      call json%get('OUTPUT.islice', temp_int, found)
      if (found) then; islice = temp_int; found_any = .true.; end if
      
      call json%get('OUTPUT.jslice', temp_int, found)
      if (found) then; jslice = temp_int; found_any = .true.; end if
      
      call json%get('OUTPUT.kslice', temp_int, found)
      if (found) then; kslice = temp_int; found_any = .true.; end if
      
      call json%get('OUTPUT.lislicedump', temp_logical, found)
      if (found) then; lislicedump = temp_logical; found_any = .true.; end if
      
      call json%get('OUTPUT.ljslicedump', temp_logical, found)
      if (found) then; ljslicedump = temp_logical; found_any = .true.; end if
      
      call json%get('OUTPUT.lkslicedump', temp_logical, found)
      if (found) then; lkslicedump = temp_logical; found_any = .true.; end if
      
      call json%get('OUTPUT.lmintdump', temp_logical, found)
      if (found) then; lmintdump = temp_logical; found_any = .true.; end if
      
      call json%get('OUTPUT.ltdump', temp_logical, found)
      if (found) then; ltdump = temp_logical; found_any = .true.; end if
      
      call json%get('OUTPUT.ltkedump', temp_logical, found)
      if (found) then; ltkedump = temp_logical; found_any = .true.; end if
      
      call json%get('OUTPUT.lxydump', temp_logical, found)
      if (found) then; lxydump = temp_logical; found_any = .true.; end if
      
      call json%get('OUTPUT.lxytdump', temp_logical, found)
      if (found) then; lxytdump = temp_logical; found_any = .true.; end if
      
      call json%get('OUTPUT.lydump', temp_logical, found)
      if (found) then; lydump = temp_logical; found_any = .true.; end if
      
      call json%get('OUTPUT.lytdump', temp_logical, found)
      if (found) then; lytdump = temp_logical; found_any = .true.; end if
   ...
   subroutine read_purifs_json(json, found_any)
      !-----------------------------------------------------------------|
      ...
      if (found) then; lpurif = temp_logical; found_any = .true.; end if
      
      call json%get('PURIFS.npurif', temp_int, found)
      if (found) then; npurif = temp_int; found_any = .true.; end if
      
      call json%get('PURIFS.epu', temp_real, found)
      if (found) then; epu = temp_real; found_any = .true.; end if
      
      call json%get('PURIFS.qpu', temp_real, found)
      if (found) then; qpu = temp_real; found_any = .true.; end if
   ...
   subroutine read_scalars_json(json, found_any)
      !-----------------------------------------------------------------|
      ...
      if (found) then; lreadscal = temp_logical; found_any = .true.; end if
      
      call json%get('SCALARS.lscasrc', temp_logical, found)
      if (found) then; lscasrc = temp_logical; found_any = .true.; end if
      
      call json%get('SCALARS.lscasrcl', temp_logical, found)
      if (found) then; lscasrcl = temp_logical; found_any = .true.; end if
      
      call json%get('SCALARS.lscasrcr', temp_logical, found)
      if (found) then; lscasrcr = temp_logical; found_any = .true.; end if
      
      call json%get('SCALARS.nscasrc', temp_int, found)
      if (found) then; nscasrc = temp_int; found_any = .true.; end if
      
      call json%get('SCALARS.nscasrcl', temp_int, found)
      if (found) then; nscasrcl = temp_int; found_any = .true.; end if
      
      call json%get('SCALARS.nsv', temp_int, found)
      if (found) then; nsv = temp_int; found_any = .true.; end if
   ...
   subroutine read_trees_json(json, found_any)
      !-----------------------------------------------------------------|
      ...
      if (found) then; ltrees = temp_logical; found_any = .true.; end if
      
      call json%get('TREES.ltreedump', temp_logical, found)
      if (found) then; ltreedump = temp_logical; found_any = .true.; end if
      
      call json%get('TREES.ntrees', temp_int, found)
      if (found) then; ntrees = temp_int; found_any = .true.; end if
      
      call json%get('TREES.cd', temp_real, found)
      if (found) then; cd = temp_real; found_any = .true.; end if
      
      call json%get('TREES.dec', temp_real, found)
      if (found) then; dec = temp_real; found_any = .true.; end if
      
      call json%get('TREES.dqdt', temp_real, found)
      if (found) then; dqdt = temp_real; found_any = .true.; end if
      
      call json%get('TREES.lad', temp_real, found)
      if (found) then; lad = temp_real; found_any = .true.; end if
      
      call json%get('TREES.lsize', temp_real, found)
      if (found) then; lsize = temp_real; found_any = .true.; end if
      
      call json%get('TREES.qstar', temp_real, found)
      if (found) then; qstar = temp_real; found_any = .true.; end if
      
      call json%get('TREES.r_s', temp_real, found)
      if (found) then; r_s = temp_real; found_any = .true.; end if
      
      call json%get('TREES.ud', temp_real, found)
      if (found) then; ud = temp_real; found_any = .true.; end if
   ...
   subroutine read_walls_json(json, found_any)
      !-----------------------------------------------------------------|
      ...
      if (found) then; dtfac = temp_real; found_any = .true.; end if
      
      call json%get('WALLS.fkar', temp_real, found)
      if (found) then; fkar = temp_real; found_any = .true.; end if
      
      call json%get('WALLS.iwallmoist', temp_int, found)
      if (found) then; iwallmoist = temp_int; found_any = .true.; end if
      
      call json%get('WALLS.iwallmom', temp_int, found)
      if (found) then; iwallmom = temp_int; found_any = .true.; end if
      
      call json%get('WALLS.iwallscal', temp_int, found)
      if (found) then; iwallscal = temp_int; found_any = .true.; end if
      
      call json%get('WALLS.iwalltemp', temp_int, found)
      if (found) then; iwalltemp = temp_int; found_any = .true.; end if
      
      call json%get('WALLS.lbottom', temp_logical, found)
      if (found) then; lbottom = temp_logical; found_any = .true.; end if
      
      call json%get('WALLS.lnorec', temp_logical, found)
      if (found) then; lnorec = temp_logical; found_any = .true.; end if
      
      call json%get('WALLS.lwritefac', temp_logical, found)
      if (found) then; lwritefac = temp_logical; found_any = .true.; end if
      
      call json%get('WALLS.nblocks', temp_int, found)
      if (found) then; nblocks = temp_int; found_any = .true.; end if
      
      call json%get('WALLS.nbndpts_c', temp_int, found)
      if (found) then; nbndpts_c = temp_int; found_any = .true.; end if
      
      call json%get('WALLS.nbndpts_u', temp_int, found)
      if (found) then; nbndpts_u = temp_int; found_any = .true.; end if
      
      call json%get('WALLS.nbndpts_v', temp_int, found)
      if (found) then; nbndpts_v = temp_int; found_any = .true.; end if
      
      call json%get('WALLS.nbndpts_w', temp_int, found)
      if (found) then; nbndpts_w = temp_int; found_any = .true.; end if
      
      call json%get('WALLS.nfcts', temp_int, found)
      if (found) then; nfcts = temp_int; found_any = .true.; end if
      
      call json%get('WALLS.nfctsecs_c', temp_int, found)
      if (found) then; nfctsecs_c = temp_int; found_any = .true.; end if
      
      call json%get('WALLS.nfctsecs_u', temp_int, found)
      if (found) then; nfctsecs_u = temp_int; found_any = .true.; end if
      
      call json%get('WALLS.nfctsecs_v', temp_int, found)
      if (found) then; nfctsecs_v = temp_int; found_any = .true.; end if
      
      call json%get('WALLS.nfctsecs_w', temp_int, found)
      if (found) then; nfctsecs_w = temp_int; found_any = .true.; end if
      
      call json%get('WALLS.nsolpts_c', temp_int, found)
      if (found) then; nsolpts_c = temp_int; found_any = .true.; end if
      
      call json%get('WALLS.nsolpts_u', temp_int, found)
      if (found) then; nsolpts_u = temp_int; found_any = .true.; end if
      
      call json%get('WALLS.nsolpts_v', temp_int, found)
      if (found) then; nsolpts_v = temp_int; found_any = .true.; end if
      
      call json%get('WALLS.nsolpts_w', temp_int, found)
      if (found) then; nsolpts_w = temp_int; found_any = .true.; end if
      
      call json%get('WALLS.prandtlturb', temp_real, found)
      if (found) then; prandtlturb = temp_real; found_any = .true.; end if
