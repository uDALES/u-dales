   ...
   CASE (91) !surface momentum flux
      ...
      DO j = jl, ju !u component
         DO i = il, iu
            ...
            ctm = unom(logdz, logzh, sqdz, Ribl0, fkar2) !save result and update field
   ...
   CASE (92) !surface temperature flux
      ...
      DO j = jl, ju
         DO i = il, iu
            ...
            call unoh(bcTflux, cth, logdz, logzh, sqdz, utangInt, dT, Ribl0, fkar2)
...
END SUBROUTINE wfmneutral
   ...
   ! CUDA implementations retained from the former wf_uno.f90.
      ATTRIBUTES(GLOBAL) SUBROUTINE wfuno_cuda(hi,hj,hk,iout1,iout2,iot,iomomflux,iotflux,obcTfluxA, &
                                               utang1,utang2,Tcell,Twall,z0,z0h,wforient)
         ...
         REAL,           INTENT(inout) :: iotflux(ib_d - hi:ie_d + hi, jb_d - hj:je_d + hj, kb_d - hk:ke_d + hk)
         REAL,           INTENT(inout) :: obcTfluxA
            ...
            IF (tidz == k) THEN
               DO j = tidy, je_d, stridey
                  ...
                  DO i = tidx, ie_d, stridex
                     ...
                     oldTfluxA = atomicadd(obcTfluxA, bcTflux)
                     iotflux(i, j, k) = iotflux(i, j, k) + bcTflux*dzfi_d(k)
                     iot(i, j, ku) = iot(i, j, ku) + &
   ...
   ! CUDA implementation retained from the former wfmneutral.f90.
      ATTRIBUTES(GLOBAL) SUBROUTINE wfmneutral_cuda(hi,hj,hk,iout1,iout2,iomomflux,utang1,utang2,z0,wforient)
