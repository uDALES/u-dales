...
contains
  ...
  subroutine subgrid
    ...
    use mpi
    use modglobal, only : ih,jh,kh,nsv,lmoist,ihc,jhc,khc,ltempeq
...
#if defined(_GPU)
    call closure_cuda<<<griddim,blockdim>>>(Rigc)
...
#if defined(_GPU)
  attributes(global) subroutine closure_cuda(Rigc)
     use modcuda, only: ie_d, je_d, ke_d, ih_d, jh_d, kh_d, dx2_d, dxi_d, dxiq_d, dy2_d, dyi_d, dyiq_d, &
                        ...
                        tidandstride
     implicit none
     real, value, intent(in) :: Rigc
     integer :: i, j, k, im, ip, jm, jp, km, kp
     integer :: tidx, tidy, tidz, stridex, stridey, stridez
     real    :: strain2, mlen, a11, a12, a13, a21, a22, a23, a31, a32, a33, aa, b11, b12, b13, b22, b23, b33, bb, du0dz, dv0dz, Rig
           ...
           do k = tidz, ke_d, stridez
              ...
              do j = tidy, je_d, stridey
                 ...
                 do i = tidx, ie_d, stridex
                    ...
                    du0dz = 0.5*( (u0_d(i,j,kp)+u0_d(ip,j,kp)) - (u0_d(i,j,km)+u0_d(ip,j,km)) )/(dzh_d(kp)+dzh_d(k))
                    dv0dz = 0.5*( (v0_d(i,j,kp)+v0_d(i,jp,kp)) - (v0_d(i,j,km)+v0_d(i,jp,km)) )/(dzh_d(kp)+dzh_d(k))
                    Rig = ( (grav_d/thl0_d(i,j,k)) * dthvdz_d(i,j,k) ) / (du0dz**2 + dv0dz**2 + 1.e-10)

                    ekm_d(i,j,k) = ekm_d(i,j,k) * sqrt( 1.0 - min(max(Rig,0.0),Rigc) / Rigc )
                 end do
              end do
           end do
        end if
  ...
  subroutine closure
    ...
    use modglobal,   only : ib,ie,jb,je,kb,ke,delta,grav,dxi,dxiq,dx2,dy2,dyi,dyiq, &
                            dzf,dzf2,dzfi,dzhi,numol,prandtlmoli,dzfiq,lbuoyancy,dzh,ltempeq
    ! use modglobal, only : ekmin, lles, rk3step ! Required by commented model gating and viscosity limits below.
    ! use modglobal, only : jmax, numoli ! Required by the commented wall-damping implementation below.
    use modfields,   only : dthvdz,e120,u0,v0,w0,thl0 !,mindist, wall, shear
    use modsurfdata, only : thvs !,dudz, dvdz
    use modmpi,      only : excjs, slabsumi !, myid
    ! use modboundary, only : closurebc
    implicit none

    real    :: strain2,mlen,a11,a12,a13, &
         a21,a22,a23,a31,a32,a33,aa,b11,b12,b13,b22, &
         b23,b33,bb,du0dz,dv0dz,Rig
       ...
       do k = kb,ke
         ...
         do j = jb,je
           ...
           do i = ib,ie        ! aij = du_j / dx_i
             ...
             if (bb < 1.e-8) then ! this tolerance is hard coded here as per the Vremann model
               ekm(i,j,k) = 0.
             else
