...
contains
  subroutine initdriver
    use modglobal, only : ih,ib,ie,jh,jb,je,kb,ke,kh,idriver,iplane,xf,lstoreplane,nstore,Uinf,ltempeq,lmoist,pi,zf,zh,driverstore,tdriverstart,tdriverdump,timeleft,dtdriver
...
!         tdriverstart = timee   !Update tdriverstart to the actual recorded value
         if(myid==0) then
            ...
            write(6,'(A,F9.2,A,I4,A,F12.9)') 'Starting time: ',tdriverstart,' Stored time steps: ',driverstore,'     Inlet record intervals: ',dtdriver
         endif
      end if
      if(rk3step==3) then
...
!      qtmdriver = qt0driver
  ...
  subroutine writedriverfile
    use modglobal, only : timee,tdriverstart,ib,ie,ih,jb,je,jh,kb,ke,kh,cexpnr,ifoutput,nstore,ltempeq,lmoist,driverstore
    use modfields, only : u0, v0, w0, e120, thl0, qt0, um
    use modmpi,    only : cmyid,myid
    use modinletdata, only : storetdriver,storeu0driver,storev0driver,storew0driver,storethl0driver,storeqt0driver,nfile,nstepreaddriver
!
    implicit none

    integer :: fileid, IOS
    integer :: i,j,k,n
    integer :: filesizet, filesizev, filesizetest1, filesizetest2
    character(15) :: name
    logical :: lexist
...
!      end if
      ...
      if(myid==0) then
        ...
        write(6,'(A,I4)') 'Inlet driver step: ', nstepreaddriver
      end if
...
!!     '(F8.4)'
      write(11,rec=nstepreaddriver)  (v0(irecydriver-1,:,:))
      close (unit=11)
!  
      name = 'wdriver_   .'
!!    write (name(13:16)  ,'(i4.4)') nfile
...
!!    write(ifoutput)  (((storew0driver (j,k,n),j=jb,je),k=kb,ke+1),n=1,nstore)
      write(11,rec=nstepreaddriver)  (w0(irecydriver-1,:,:))
      close (unit=11)
...
!!    write(ifoutput)  (((storew0driver (j,k,n),j=jb,je),k=kb,ke+1),n=1,nstore)
      write(11,rec=nstepreaddriver)  (e120(irecydriver-1,:,:))
      close (unit=11)
...
!!    write(ifoutput)  (((storew0driver (j,k,n),j=jb,je),k=kb,ke+1),n=1,nstore)
        write(11,rec=nstepreaddriver)  (thl0(irecydriver-1,:,:))
        close (unit=11)
      end if
...
!!    write(ifoutput)  (((storew0driver (j,k,n),j=jb,je),k=kb,ke+1),n=1,nstore)
        write(11,rec=nstepreaddriver)  (qt0(irecydriver-1,:,:))
        close (unit=11)
      end if
!
  ...
  subroutine readdriverfile
    ...
    use modglobal, only : ib,jb,je,jmax,kb,ke,kh,cexpnr,ifinput,driverstore,ltempeq,lmoist,zh,jgb,jge,jh,driverjobnr 
    use modmpi,    only : cmyid,myid,nprocs,slabsum,excjs
    use modinletdata, only : storetdriver,storeu0driver,storev0driver,storew0driver,storee120driver,storethl0driver,storeqt0driver,nfile

    implicit none

    integer :: filen,filee
    integer :: fileid, IOS, filesize
...
!          inquire(file=name,recl=filesize)
  ...
  use modglobal,      only : idriver,lstoreplane,ltempeq,lmoist
  ...
  if (idriver==1) then
    ...
    deallocate(storetdriver, storeu0driver,storev0driver,storew0driver,storee120driver,u0driver,v0driver,w0driver,e120driver)
    if (ltempeq ) then
