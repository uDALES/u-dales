    ...
    if (ltimedepsw) then
      ...
      if (myid == 0) then
        ...
        do t = 1,ntimedepsw
          ...
          chmess1 = "#"
          ierr = 1 ! not zero
          do while (.not.(chmess1 == "#" .and. ierr == 0)) !search for the next line consisting of "# time", from there onwards the fluxes will be read
            read(ifinput,*,iostat=ierr) chmess1, timesw(t)
            !if (ierr < 0) then
              !stop 'STOP: No time dependend data for end of run'
            !end if
          end do

         read (ifinput, *) (netswt(n,t), n = 1,nfcts)
