...
!> Do the xz crosssections and dump them to file
  ...
  allocate(thv0(2:i1,1:kmax))
    ...
    write(ifoutput,'(es11.4)') ((thv0(i,k),i=2,i1),k=1,kmax)
    close(ifoutput)
    ...
    if (lnetcdf) then
      ...
      call writestat_nc(ncid1,7,ncname1(1:7,:),vars,nrec1,imax,kmax)
      deallocate(vars)
    end if
    deallocate(thv0)
