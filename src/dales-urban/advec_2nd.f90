...
!! \par Revision list
!! Added pressure gradient term (for IBM model)
!! variable mesh in x-direction now (also) possible
!! \par ILS13 040615, Jasper Tomas 040615
!!
!! ILS13 040615
!!
...
subroutine advecc_2nd(hi,hj,hk,putin,putout)
  ...
  use modglobal, only : ih,jh,kh,kb,ke,ib,ie,jb,je,dxf,dxhi,dxfi5,dyi5,dzf,dzfi5,dzhi,libm
  use modfields, only : u0, v0, w0
  use modibm, only : nxwallsnorm,nzwallsnorm,nywallsm,nywallsp,ywallsm,ywallsp,&
                     xwallsnorm,zwallsnorm
  implicit none

!  real, dimension(ib-ih:ie+ih,jb-jh:je+jh,kb-kh:ke+kh), intent(in)  :: putin !< Input: the cell centered field
...
! Set advective fluxes to zero for IBM walls
  if (libm == .true.) then
    do n=1,nxwallsnorm
      ...
      kl = xwallsnorm(n,4)
      ku = xwallsnorm(n,5)
      putout(im,jl:ju,kl:ku) = putout(im,jl:ju,kl:ku) + ( &
!              u0(i,j,k) * ( putin(i,jl:ju,kl:ku)*dxf(im) + putin(im,jl:ju,kl:ku)*dxf(i) ) *dxhi(i) &
              u0(i,jl:ju,kl:ku) * ( putin(i,jl:ju,kl:ku)*dxf(im) + putin(im,jl:ju,kl:ku)*dxf(i) ) *dxhi(i) &
              )* dxfi5(im)
      putout(i,jl:ju,kl:ku) = putout(i,jl:ju,kl:ku) + ( &
!             -u0(i ,j,k) * ( putin(im,jl:ju,kl:ku)*dxf(i) + putin(i,jl:ju,kl:ku)*dxf(im) ) *dxhi(i)   &          ! d(uc)/dx
             -u0(i ,jl:ju,kl:ku) * ( putin(im,jl:ju,kl:ku)*dxf(i) + putin(i,jl:ju,kl:ku)*dxf(im) ) *dxhi(i)   &          ! d(uc)/dx
              )* dxfi5(i)
    end do
    ...
    do n=1,nywallsm
      ...
      putout(il:iu,j,kl:ku) = putout(il:iu,j,kl:ku)    +( &
              v0(il:iu,jp  ,kl:ku) * ( putin(il:iu,jp,kl:ku) + putin(il:iu,j,kl:ku) ) &          !d(vc)/dy
              )* dyi5
    end do
    do n=1,nywallsp
      j  = ywallsp(n,1)
      ...
      putout(il:iu,j,kl:ku) = putout(il:iu,j,kl:ku)    +( &
              -v0(il:iu,j  ,kl:ku) * ( putin(il:iu,jm,kl:ku) + putin(il:iu,j,kl:ku) ) &          !d(vc)/dy
              )* dyi5
    ...
    do n=1,nzwallsnorm
      ...
      ju = zwallsnorm(n,5)
      if (k > kb) then
        putout(il:iu,jl:ju,km) = putout(il:iu,jl:ju,km)    +( &
!                  w0(i,j,k) * (putin(il:iu,jl:ju,k)*dzf(km) + putin(il:iu,jl:ju,km)*dzf(k) ) * dzhi(k) &
                  w0(il:iu,jl:ju,k) * (putin(il:iu,jl:ju,k)*dzf(km) + putin(il:iu,jl:ju,km)*dzf(k) ) * dzhi(k) &
                  ) * dzfi5(km)
      end if
      putout(il:iu,jl:ju,k) = putout(il:iu,jl:ju,k)      +( &
!               -w0(i,j,k ) * (putin(il:iu,jl:ju,km)*dzf(k) + putin(il:iu,jl:ju,k)*dzf(km) ) * dzhi(k) &
               -w0(il:iu,jl:ju,k ) * (putin(il:iu,jl:ju,km)*dzf(k) + putin(il:iu,jl:ju,k)*dzf(km) ) * dzhi(k) &
                ) * dzfi5(k)
    end do
  end if
...
subroutine advecu_2nd(putin, putout)
  ...
  use modglobal, only : ih,ib,ie,jb,je,jh,kb,ke,kh,dxhiq,dyiq,dzf,dzfi5,dzhi,dxhi
