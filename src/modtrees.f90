...
!        ladz(1:ntree_max) = (/  0.85/20, 0.885/20, 0.92/20, 1.02/20, 1.12/20,  1.31/20, 1.5/20, 1.85/20, 2.2/20, 2.45/20, 2.7/20, 2.85/20, 3./20, 2.975/20, 2.95/20, 2.85/20, 2.75/20, 2.325/20, 2./20, 1./20 /) !LAI =2 from Shaw, 1992
        ladz(1:ntree_max) = (/  0.1, 0.11, 0.12, 0.14, 0.16, 0.19, 0.22, 0.265, 0.31, 0.335, 0.36, 0.37, 0.38, 0.3725, 0.365, 0.3375, 0.31, 0.2375, 0.165, 0.0875 /) !LAI =2 from Shaw, 1992

      else
        ladz(1:ntree_max+1) = lad !+1 cos how clai is calculated using lad at cell faces so we need lad at top face
      end if

      ! clai at cell faces using lad at cell centre
      do k = 1,ntree_max
        clai(k) = sum(0.5*(ladz(k:ntree_max)+ladz(k+1:ntree_max+1))*(dzf(k:tree(1,6))))
      end do
    ...
    call MPI_BCAST(ladz  , ke+1,MY_REAL ,0,comm3d,mpierr)
...
!                 write(*,*) thlm(i,j,k), s, D, e_vap, e_sat, lad, ntree_max, r_a, r_s, gam
                 ...
                 qe = (s*(qa(ntree_max-(ku-k))/(dzf(k)*ladz(ntree_max-(ku-k)))) + 2*rhoa*cp*(1/r_a)*D) / (s + (gam*2*(r_a+r_s)/r_a))
