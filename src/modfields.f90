...
#if defined(_GPU)
  real, allocatable, pinned, target :: tau_x(:,:,:), tau_y(:,:,:), tau_z(:,:,:), thl_flux(:,:,:)
#else
...
! TKE budget results (written to tkedump): !tg3315
  ...
  real              :: thlsrcdt = 0.              !< time derivative used by the potential-temperature controller
  real              :: dgdt = 0.                   !< time derivative of the imposed pressure gradient
...
contains
  ...
  subroutine initfields
    ...
    use modglobal, only : ib,ie,jb,je,ih,jh,kb,ke,kh,jtot,nsv,&
         ihc,jhc,khc,ltdump,lmintdump,lytdump,lxytdump,ltkedump,lscasrcr,ltreedump
