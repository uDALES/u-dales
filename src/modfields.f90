...
module modfields
  ...
  real, allocatable, target :: div(:,:,:)      !<   liq. water pot. temperature at time step t
  real, allocatable, target :: divf(:,:,:)      !<   liq. water pot. temperature at time step t
  ...
  real, allocatable, target :: tau_x(:,:,:)        !<   x-component of shear stress
  real, allocatable, target :: tau_y(:,:,:)        !<   y-component of shear stress
  real, allocatable, target :: tau_z(:,:,:)        !<   z-component of shear stress
  real, allocatable, target :: thl_flux(:,:,:)      !<   volumetric heating
...
contains
  ...
  subroutine initfields
    ...
    allocate(div(ib:ie,jb:je,kb:ke))
    allocate(divf(ib:ie,jb:je,kb:ke))

    allocate(tau_x(ib-ih:ie+ih,jb-jh:je+jh,kb-kh:ke+kh))
    allocate(tau_y(ib-ih:ie+ih,jb-jh:je+jh,kb-kh:ke+kh))
    allocate(tau_z(ib-ih:ie+ih,jb-jh:je+jh,kb-kh:ke+kh))
    allocate(thl_flux(ib-ih:ie+ih,jb-jh:je+jh,kb-kh:ke+kh))
