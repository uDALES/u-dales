...
module stats
  use modglobal,  only : cexpnr, ltempeq, lmoist, lchem, nsv, rk3step, &
                         ...
                         nreceptor_heights, receptor_heights, &
                         dt, runtime, btime, &
                         k1, JNO2
  ...
  use modstat_nc, only : ncinfo, open_nc, define_nc, writestat_dims_nc, writestat_nc, &
                         ...
                         write_pedestrian_wind_metadata_nc, write_pedestrian_wind_heights_nc, nc_fillvalue
  use netcdf
  ...
  real, allocatable :: ws_localt(:,:,:), ws_10t(:,:), ws_now(:,:), wind_heights(:)
  logical, allocatable :: ws_local_valid(:,:,:), ws_10_valid(:,:), ws_local_available(:)
  integer, allocatable :: ws_local_klo(:), ws_local_khi(:)
  real, allocatable :: ws_local_weight(:)
  integer :: ws_10_klo, ws_10_khi
  real    :: ws_10_weight
  logical :: ws_10_available
  ...
  contains
    ...
    subroutine stats_prepare
      ...
      if(lstatstavgdump) then
        ...
        if (ltdump .or. ltkslicedump) call stats_allocate_tavg_pedestrian_wind
      ...
      if (ltavg3d) then
        ...
        if (lmoist)  tVarsCount = tVarsCount + 5
      ...
      if (lxytdump) then
        ...
        if (lmoist)  xytVarsCount = xytVarsCount + 6
      ...
      if (lxydump) then
        ...
        if (lmoist)  xyVarsCount = xyVarsCount + 5
      ...
      if (lytdump) then
        ...
        if (lmoist)  ytVarsCount = ytVarsCount + 6
      ...
      if (lydump) then
        ...
        if (lmoist)  yVarsCount = yVarsCount + 5
    ...
    subroutine stats_main
      ...
      if (tsamplep >= tsample) then        ! at every stats sampling instance
        ...
        if(lstatstavgdump) then
          ...
          if (ltdump .or. ltkslicedump) call stats_compute_tavg_pedestrian_wind
      ...
      if ( (lstatstavgdump .or. ltreedump) &
        ...
        if(lstatstavgdump) then
          ...
          if (ltdump .or. ltkslicedump) call stats_reset_tavg_pedestrian_wind
    ...
    subroutine stats_allocate_tavg_pedestrian_wind
      ...
      if (nreceptor_heights > 0) then
        allocate(wind_heights(nreceptor_heights))
        wind_heights = receptor_heights(1:nreceptor_heights)
      else
        allocate(wind_heights(1))
        wind_heights(1) = receptor_height
      end if
      allocate(ws_localt(ib:ie,jb:je,1:size(wind_heights))); ws_localt = 0.
      allocate(ws_10t   (ib:ie,jb:je)); ws_10t    = 0.
      allocate(ws_now   (ib:ie,jb:je)); ws_now    = 0.
      allocate(ws_local_valid(ib:ie,jb:je,1:size(wind_heights))); ws_local_valid = .false.
      allocate(ws_local_klo(size(wind_heights)), ws_local_khi(size(wind_heights)))
      allocate(ws_local_weight(size(wind_heights)), ws_local_available(size(wind_heights)))
      allocate(ws_10_valid   (ib:ie,jb:je)); ws_10_valid    = .false.

      do h = 1, size(wind_heights)
        call stats_find_vertical_bracket(wind_heights(h), ws_local_klo(h), ws_local_khi(h), &
                                         ws_local_weight(h), ws_local_available(h))
        if (ws_local_available(h)) then
          if (ws_local_klo(h) == ws_local_khi(h)) then
            ws_local_valid(:,:,h) = IIc(ib:ie,jb:je,ws_local_klo(h)) == 1
          else
            ws_local_valid(:,:,h) = (IIc(ib:ie,jb:je,ws_local_klo(h)) == 1) .and. &
                                     (IIc(ib:ie,jb:je,ws_local_khi(h)) == 1)
          end if
        else if (myid == 0) then
          write(0,'(a,f10.3,a)') 'WARNING: ws_local cannot be sampled at ', &
                                 wind_heights(h), ' m: outside the scalar-level range.'
        end if
      end do
    ...
    subroutine stats_ncdescription_tavg_vel
      ...
      call ncinfo( tVars(ctrt+ 5,:), 'pabs'    , 'Hydrostatic absolute pressure', 'Pa'      , 'tttt' )

      call ncinfo( tVars(ctrt+ 6,:), 'upwp'    , 'Turbulent momentum flux'    , 'm^2/s^2'   , 'mtmt' )
      call ncinfo( tVars(ctrt+ 7,:), 'vpwp'    , 'Turbulent momentum flux'    , 'm^2/s^2'   , 'tmmt' )
      call ncinfo( tVars(ctrt+ 8,:), 'upvp'    , 'Turbulent momentum flux'    , 'm^2/s^2'   , 'mmtt' )

      call ncinfo( tVars(ctrt+ 9,:), 'upup'    , 'u variance - cell centered' , 'm^2/s^2'   , 'tttt' )
      call ncinfo( tVars(ctrt+10,:), 'vpvp'    , 'v variance - cell centered' , 'm^2/s^2'   , 'tttt' )
      call ncinfo( tVars(ctrt+11,:), 'wpwp'    , 'w variance - cell centered' , 'm^2/s^2'   , 'tttt' )
      call ncinfo( tVars(ctrt+12,:), 'tke'     , 'TKE - cell centered'        , 'm^2/s^2'   , 'tttt' )

      call ncinfo( tVars(ctrt+13,:), 'usgs'    , 'SGS u flux'                 , 'm^2/s^2'   , 'mtmt' )
      call ncinfo( tVars(ctrt+14,:), 'vsgs'    , 'SGS v flux'                 , 'm^2/s^2'   , 'tmmt' )
      call ncinfo( tVars(ctrt+15,:), 'wsgs'    , 'SGS w flux'                 , 'm^2/s^2'   , 'ttmt' )
      ctrt = ctrt+15
    ...
    subroutine stats_allocate_tavg_moist
      ...
      allocate(qlt(ib:ie,jb:je,kb:ke+kh))   ; qlt    = 0.;
    ...
    subroutine stats_ncdescription_tavg_moist
      ...
      call ncinfo( tVars(ctrt+5,:) , 'ql'      , 'Liquid-water specific humidity', 'kg/kg'    , 'tttt' )
      ctrt = ctrt+5
    ...
    subroutine stats_createnc_tavg
      ...
      if (myidy==0) then
        ...
        call stats_define_pedestrian_wind(ncidt)
      end if
    end subroutine stats_createnc_tavg

    subroutine stats_define_pedestrian_wind(ncid)
      implicit none
      integer, intent(in) :: ncid

      if (size(wind_heights) == 1) then
        call define_nc(ncid, size(pedestrianWindVars,1), pedestrianWindVars)
        call write_pedestrian_wind_metadata_nc(ncid, wind_heights(1))
      else
        call define_nc(ncid, 1, pedestrianWindVars(2:2,:))
        call write_pedestrian_wind_heights_nc(ncid, wind_heights)
      end if
    end subroutine stats_define_pedestrian_wind
    ...
    subroutine stats_reset_tavg_moist
      implicit none
      qtt = 0.; qlt = 0.; qttk = 0.; wqttk = 0.; qtqtt = 0.; qtsgst = 0.
    ...
    subroutine stats_init_xytavg_vel
      ...
      call ncinfo( xytVars(ctrxyt+21,:), 'pabs'     , 'Hydrostatic absolute pressure', 'Pa'      , 'tt' )
      ctrxyt = ctrxyt+21
    ...
    subroutine stats_init_xytavg_moist
      ...
      call ncinfo( xytVars(ctrxyt+ 6,:), 'ql'       , 'Liquid-water specific humidity', 'kg/kg'    , 'tt' )
      ctrxyt = ctrxyt+6
    ...
    subroutine stats_init_xyavg_vel
      ...
      call ncinfo( xyVars(ctrxy+17,:), 'pabs'     , 'Hydrostatic absolute pressure', 'Pa'      , 'tt' )
      ctrxy = ctrxy+17
    ...
    subroutine stats_init_xyavg_moist
      ...
      call ncinfo( xyVars(ctrxy+ 5,:), 'ql'       , 'Liquid-water specific humidity', 'kg/kg'    , 'tt' )
      ctrxy = ctrxy+5
    ...
    subroutine stats_init_ytavg_vel
      ...
      call ncinfo( ytVars(ctryt+12,:), 'pabs'     , 'Hydrostatic absolute pressure', 'Pa'      , 't0tt' )
      ctryt = ctryt+12
    ...
    subroutine stats_init_ytavg_moist
      ...
      call ncinfo( ytVars(ctryt+ 6,:), 'ql'       , 'Liquid-water specific humidity', 'kg/kg'    , 't0tt' )
      ctryt = ctryt+6
    ...
    subroutine stats_init_yavg_vel
      ...
      call ncinfo( yVars(ctry+ 9,:), 'pabs'     , 'Hydrostatic absolute pressure', 'Pa'      , 't0tt' )
      ctry = ctry+9
    ...
    subroutine stats_init_yavg_moist
      ...
      call ncinfo( yVars(ctry+ 5,:), 'ql'       , 'Liquid-water specific humidity', 'kg/kg'    , 't0tt' )
      ctry = ctry+5
    ...
    subroutine stats_compute_tavg_pedestrian_wind
      ...
      do h = 1, size(wind_heights)
        if (.not. ws_local_available(h)) cycle
        call stats_horizontal_speed_at_height(ws_local_klo(h), ws_local_khi(h), ws_local_weight(h), &
                                              ws_local_valid(:,:,h), ws_now)
        call stats_compute_tavg(ws_localt(:,:,h), ws_now)
      end do
    ...
    subroutine stats_compute_tavg_moist
      ...
      call stats_compute_tavg(qlt   , ql0(ib:ie,jb:je,kb:ke+kh) )
    ...
    subroutine stats_compute_xytavg_moist
      ...
      call spatial_avg(qlxyt,qlt(ib:ie,jb:je,kb:ke+kh),kb,ke,IIc(ib:ie,jb:je,kb:ke+kh),IIcs(kb:ke+kh),.false.)
    ...
    subroutine stats_compute_xyavg_moist
      ...
      call spatial_avg(qlxy,ql0(ib:ie,jb:je,kb:ke+kh),kb,ke,IIc(ib:ie,jb:je,kb:ke+kh),IIcs(kb:ke+kh),.false.)
    ...
    subroutine stats_compute_ytavg_moist
      ...
      call spatial_avg(qlyt,qlt(ib:ie,jb:je,kb:ke),IIc(ib:ie,jb:je,kb:ke),IIct)
    ...
    subroutine stats_compute_yavg_moist
      ...
      call spatial_avg(qly,ql0(ib:ie,jb:je,kb:ke),IIc(ib:ie,jb:je,kb:ke),IIct)
    ...
    subroutine stats_write_tavg
      ...
      call stats_write_pedestrian_wind(ncidt, nrect)
    end subroutine stats_write_tavg

    subroutine stats_write_pedestrian_wind(ncid, record)
      implicit none
      ...
      if (size(wind_heights) == 1) then
        ...
        fld2d = ws_localt(:,:,1)
        where (.not. ws_local_valid(:,:,1)) fld2d = nc_fillvalue
        call writeoffset_2d(ncid, 'ws_local', fld2d, record, xdim, ydim)
        deallocate(fld2d)
      else
        allocate(fld3d(ib:ie,jb:je,1:size(wind_heights)))
        fld3d = ws_localt
        where (.not. ws_local_valid) fld3d = nc_fillvalue
        call writeoffset(ncid, 'ws_local', fld3d, record, xdim, ydim, size(wind_heights))
        deallocate(fld3d)
      end if
      ...
      call writeoffset_2d(ncid, 'ws_10', fld2d, record, xdim, ydim)
      deallocate(fld2d)
    end subroutine stats_write_pedestrian_wind
    ...
    subroutine stats_write_tavg_kslice
      ...
      deallocate(fld, tmp)
      call stats_write_pedestrian_wind(ncidtks, nrectks)
    ...
    subroutine stats_createnc_tkslice
      ...
      if (myidy == 0) then
        ...
        call stats_define_pedestrian_wind(ncidtks)
      end if
    ...
    subroutine stats_write_xytavg_vel
      ...
      call writestat_nc(ncidxyt, 'pabs'    , presf(kb:ke)      , nrecxyt, zdim)
    ...
    subroutine stats_write_xytavg_moist
      ...
      call writestat_nc(ncidxyt, 'ql'      , qlxyt(kb:ke)      , nrecxyt, zdim)
    ...
    subroutine stats_write_xyavg_vel
      ...
      call writestat_nc(ncidxy, 'pabs'    , presf(kb:ke)     , nrecxy, zdim)
    ...
    subroutine stats_write_xyavg_moist
      ...
      call writestat_nc(ncidxy, 'ql'      , qlxy(kb:ke)      , nrecxy, zdim)
    ...
    subroutine stats_write_ytavg_vel
      ...
      call writestat_nc(ncidyt, 'pabs' , spread(presf(kb:ke), 1, xdim), nrecyt, xdim, zdim)
    ...
    subroutine stats_write_ytavg_moist
      ...
      call writestat_nc(ncidyt, 'ql'     , qlyt     , nrecyt, xdim, zdim)
    ...
    subroutine stats_write_yavg_vel
      ...
      call writestat_nc(ncidy, 'pabs' , spread(presf(kb:ke), 1, xdim), nrecy, xdim, zdim)
    ...
    subroutine stats_write_yavg_moist
      ...
      call writestat_nc(ncidy, 'ql'     , qly     , nrecy, xdim, zdim)
    ...
    subroutine stats_exit
      ...
      if (ltdump .or. ltkslicedump) then
        deallocate(ws_localt,ws_10t,ws_now,ws_local_valid,ws_10_valid,wind_heights, &
                   ws_local_klo,ws_local_khi,ws_local_weight,ws_local_available)
      end if
      if (ltavg3d .or. lytdump) then
      ...
      if (lxytdump) then
        ...
        if (lmoist)  deallocate(qtxyt,qlxyt,wpqtpxytk,wqtxytk,qtpqtpxyt,qtsgsxyt)
      ...
      if (lxydump) then
        ...
        if (lmoist)  deallocate(qtxy,qlxy,wpqtpxyk,wqtxyk,qtxyk,qtsgsxy)
      ...
      if (lytdump) then
        ...
        if (lmoist)  deallocate(qtyt,qlyt,wpqtpytk,wqtytk,qtpqtpyt,qtsgsyt)
      ...
      if (lydump) then
        ...
        if (lmoist)  deallocate(qty,qly,wpqtpyk,wqtyk,qtyk,qtsgsy)
