...
The shell wrappers in `tools/` automate the whole simulation lifecycle
(build -> copy_inputs -> write_inputs -> execute -> gather) and are the
PREFERRED interface — use them before hand-rolling cmake/mpiexec/qsub
commands. They read a per-case `config.sh` and support the two production
platforms (ICL CX3 = PBS, ARCHER2 = Slurm) with a documented recipe for
porting to others. Details: `tools/README.md`.
