  ...
  if(rk3step == 1) then
      ...
      if (ladaptive) then
        ...
        if ( pecletold>0) then
          if (abs(courtot-courold)/courold<0.1 .and. (abs(peclettot-pecletold)/pecletold<0.1)) then
            dt = minval((/runtime-timee+btime,dt*courant/courtot,dt*peclet/peclettot,dt_lim/))
    ...
    ! Normal time loop
      ...
      if (ladaptive) then
        ...
        call MPI_ALLREDUCE(courtotl,courtot,1,MY_REAL,MPI_MAX,comm3d,mpierr)
        call MPI_ALLREDUCE(peclettotl,peclettot,1,MY_REAL,MPI_MAX,comm3d,mpierr)
        dt = minval((/runtime-timee+btime+1e-3, dt_lim, dt*courant/courtot , dt*peclet/peclettot /))
        dt_lim = runtime-timee+btime
        timee   = timee  + dt
        ntimee  = ntimee + 1
        ntrun   = ntrun  + 1
