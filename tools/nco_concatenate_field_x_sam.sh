...
## call: dumptype, xm-variables(list separated by comma, no space, include xm), out.nc
## e.g. nco_concatenate_field.sh fielddump u,xm out.nc
...
if (( n > 1 )) ; then
    ...
    if [ $xmparam = "xm" ] ; then
        ...
        for file in ${dumps}.*.*.nc; do
            ncpdq -64 -a xt,time,yt,ym,zt,zm -C -x -v ${xmparam} ${file} "tmp${file#${dumps}}"
        done
        ...
        ncpdq -64 -a time,zt,zm,xt,xm,yt,ym fieldtmp1.nc fieldtmp3.nc
        ncpdq -64 -a time,zt,zm,xt,xm,yt,ym fieldtmp2.nc ${outfile}
