...
# Faces whose centre sits within this height above z=0 still count as ground:
# STL float noise puts nominally flat ground at ~1e-9..1e-5 m, and a strict
# z > 0 test scatters ground triangles into the buildings group, painting
# most of the ground in the building colour (issue #337). No real building
# is a millimetre tall, so 1 mm cleanly separates noise from geometry.
GROUND_Z_TOL = 1e-3  # metres


def _is_building_faces(face_centers: np.ndarray) -> np.ndarray:
    """Boolean mask of faces whose centre lies above the ground tolerance."""
    return np.asarray(face_centers)[:, 2] > GROUND_Z_TOL
...
class UDVis:
    ...
    def show_geometry(
        ...
        is_building = _is_building_faces(face_centers)

        # No "flat" lighting preset here: these meshes carry uniform solid
        # colours, so ambient-only lighting collapses the Plotly view into an
        # unshaded silhouette (issue #337). Default lighting shades them per
        # face, matching the PyVista backend. The outline plots keep "flat"
        # because their shading is pre-baked into two-tone face colours.
        meshes = []
        if color_buildings:
            if show_ground and np.any(~is_building):
                meshes.append(MeshPrimitive(vertices, faces[~is_building],
                                            solid_color=GROUND_RGB, name="ground"))
            if np.any(is_building):
                meshes.append(MeshPrimitive(vertices, faces[is_building],
                                            solid_color=BUILDING_RGB, name="buildings"))
        else:
        ...
        if show_edges:
            ...
            if len(segments):
                # PyVista: thin and semi-transparent, so dense wireframes read
                # as shading instead of a black mass (issue #337). Plotly keeps
                # its original bold opaque lines - its WebGL renderer turns
                # thin translucent lines into stipple.
                scene.lines.append(LineSet(vertices, segments, color="black", width=1.0,
                                           opacity=0.4, lift_ground=True,
                                           width_plotly=2.0, opacity_plotly=1.0))
    ...
    def show_geometry_outline(
        ...
        plotly.graph_objects.Figure or pyvista.Plotter or None
        """
        scene = self._outline_scene(angle_threshold=angle_threshold,
    ...
    def _outline_scene(self, angle_threshold: float = 45.0, show_ground: bool = True,
                       color_buildings: bool = False, line_width: float = 2.0):
        """Geometry Scene with outline edges, shared by the outline plots.

        Returns ``None`` (after a warning) when the mesh yields no outline
        edges; raises when no geometry is loaded.
        """
        if self.geom is None or getattr(self.geom, "stl", None) is None:
            raise ValueError("No geometry loaded. Cannot visualize.")
        ...
        return Scene(
            meshes=meshes,
            lines=[LineSet(vertices, segments, color="black", width=line_width,
                           lift_ground=True)],
    ...
    def _base_overlay_scene(self, geom, title: Optional[str] = None) -> Scene:
        ...
        if len(segments):
            # Thin, semi-transparent full wireframe in PyVista; original bold
            # opaque lines in Plotly - see show_geometry for the rationale
            # (issue #337).
            scene.lines.append(LineSet(vertices, segments, color="black", width=1.0,
                                       opacity=0.4, lift_ground=True,
                                       width_plotly=1.5, opacity_plotly=1.0))
        return scene
    ...
    def plot_veg(self, veg: Optional[Dict[str, Any]] = None, show: bool = False,
        ...
        points = self._resolve_veg_points(veg, "plot_veg")
        if points is None:
            return None
    ...
    def _resolve_veg_points(self, veg: Optional[Dict[str, Any]], caller: str):
        """Vegetation points from the argument or the case, subsampled.

        Returns an (n, 3) int array, or ``None`` after a single warning when
        the case has no usable vegetation data (shared by the vegetation
        plots so they degrade identically on cases without vegetation).
        """
        if veg is None:
        ...
        if len(points) > max_points:
            ...
            logger.info("%s: showing %d of %d points", caller, max_points, len(veg['points']))
        return points

    def plot_veg_outline(self, veg: Optional[Dict[str, Any]] = None, show: bool = False,
                         backend: Optional[str] = None, color=VEG_RGB,
                         opacity: float = 1.0, line_width: float = 2.0,
                         angle_threshold: float = 45.0):
        """Plot vegetation voxels over outline-style geometry.

        Same vegetation rendering as :meth:`plot_veg` (opaque boxes filling
        their grid cells), but on the two-tone outline base of
        :meth:`show_geometry_outline` instead of the full wireframe - the
        most readable vegetation view on fine meshes (issue #337).
        ...
        show : bool, default=False
            If True, display immediately and return None. If False (default,
            like the other overlay plots), return the figure/plotter.
        backend : {"plotly", "pyvista"}, optional
            Rendering backend; defaults to this UDVis instance's backend.
        color : matplotlib colour or (r, g, b) tuple, optional
            Vegetation colour; defaults to forest green.
        ...
        plotly.graph_objects.Figure or pyvista.Plotter or None
        """
        if self.sim is None or not self.sim._lfgeom or self.sim.geom is None:
            return self._missing_plot_data("Geometry data not found for plot_veg_outline")
        points = self._resolve_veg_points(veg, "plot_veg_outline")
        if points is None:
            return None
        ...
        if scene is None:
            return None
        scene.title = f"Geometry Outline with Vegetation ({len(points)} points)"
