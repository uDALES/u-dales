...
This module mirrors directshortwave.py but JIT-compiles the ray traversal kernel.
If numba is unavailable, it falls back to the pure-Python implementation.
"""
...
from .directshortwave import _build_veg_fields, _load_veg_data
...
if nb is not None:
    ...
    def _trace_ray_numba(
        ...
        while 0 <= k < ktot and (periodic_xy or allow_outside_xy or (0 <= i < itot and 0 <= j < jtot)):
            ...
            ii = i % itot if periodic_xy else i
            jj = j % jtot if periodic_xy else j
            if inside and has_solid and solid[ii, jj, k]:
                if last_i >= 0:
...
def directshortwave(
    ...
    If numba is not available, this falls back to the pure-Python implementation.
    """
    if nb is None:
        from .directshortwave import directshortwave as directshortwave_py

        return directshortwave_py(
            sim,
            nsun=nsun,
            ...
            ray_jitter_seed=ray_jitter_seed,
            return_hit_count=return_hit_count,
